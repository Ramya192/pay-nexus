...
Dated, in-order log of every bug found/fixed, feature added, and review pass run across this build.
`README.md` stays focused on architecture, setup, and current status; this file is the detailed
"what happened and why" record — useful for understanding a specific decision, not for getting oriented
on the project (start with `README.md` for that).

Each entry follows the same pattern this whole build used: diagnose the real root cause, fix at the
source, verify with actual execution (direct invocation → `curl` → browser, or the equivalent for
non-web changes) — not just written and assumed correct.
...
## Phase 6 (Deployment) — actually done, not just scaffolded (Aug 2026, user-requested)

The repo already had a `backend/Dockerfile` and a `.github/workflows/deploy.yml` before this — neither
mentioned in any prior review pass, and neither functional (`deploy.yml` was explicitly labeled
`# Placeholder`; the Dockerfile had never been built). Real infrastructure existed too, from earlier in
this same session but likewise unreviewed: `paynexus-rg` (resource group, `indiasouthcentral`) and
`paynexus-db-ramya` (PostgreSQL Flexible Server, `Standard_B1ms` Burstable, `vector` extension already
enabled, full schema already migrated in) — `backend/.env`'s `DATABASE_URL` had been pointing at this real
Azure database the entire session; "local" testing was never purely local.

**What got built this pass:**

- **`backend/.dockerignore`** — a real gap: without it, `docker build` would have baked `backend/.env`'s
  real `OPENAI_API_KEY` and `DATABASE_URL` password straight into the image's filesystem layers. Fixed and
  confirmed (`docker run ... ls /app/.env` → not found) before ever building for real.
- **`backend/Dockerfile`** — hardened (non-root user, `HEALTHCHECK`, `PYTHONUNBUFFERED`) and switched to
  `python -m uvicorn` over the bare `uvicorn` console-script — the exact distinction that broke the first
  real deploy attempt (see below). Built and run locally twice, both times verified with a real `200` on
  `/docs` and clean logs before ever pushing anywhere.
- **Azure App Service** (`paynexus-api`, Basic B1, Linux) — first attempt used App Service's native Python
  runtime (no Docker, cheaper, no registry needed). **Failed** with
  `ModuleNotFoundError: No module named 'api'` on every restart — Oryx's zip-deploy build compresses the
  venv into a randomized `/tmp/<hash>` path on every cold start, and neither `python -m uvicorn` nor an
  explicit `PYTHONPATH` app setting fixed it (Oryx's own generated startup script recomputes and overwrites
  `PYTHONPATH` after the app setting is applied — confirmed via two separate fix attempts, both hitting the
  identical error). Rather than keep debugging Oryx internals with no direct filesystem access, switched to
  the already-tested Docker image instead — a known-good path beats an unknown one. **Docker Hub** (free
  tier, avoiding Azure Container Registry's ~$5/month) hosts the image; App Service runs it as "Web App for
  Containers." Confirmed via real startup log: `"Site startup probe succeeded after 11.0388593 seconds"` →
  `"Site started."`
- **Azure Static Web Apps** (`paynexus-web`, free tier) — hosts the frontend.
- **`frontend/src/api/client.ts` / `api/chat.ts`** — a real, previously-unfixed gap: `client.ts` had an
  explicit `// Set a real base URL here once the backend has a deployed address` comment, and `chat.ts`'s
  SSE streaming call used a bare `fetch("/chat", ...)` that only ever worked by accident, via Vite's dev
  proxy — neither would have worked against a real deployed backend on a different host. Both now use
  `VITE_API_BASE_URL` (`.env.production`, Vite-inlined at build time), pointing at the real
  `paynexus-api.azurewebsites.net`.
- **`CORS_ORIGINS`** updated on the backend to allow the real deployed frontend origin, not just
  `localhost:5173` — confirmed with a real CORS preflight request showing the correct
  `Access-Control-Allow-Origin` header back.
- **`.github/workflows/deploy.yml`** — replaced the placeholder with two real jobs: build+push the backend
  image to Docker Hub (App Service's Continuous Deployment webhook, set up once via
  `az webapp deployment container config --enable-cd true`, picks up the new `:latest` tag automatically —
  no explicit restart step needed in the workflow itself), and build+deploy the frontend to Static Web Apps.
  **Verified with a real run, not just written**: pushed, both jobs went green
  (`deploy-backend` and `deploy-frontend`, ~1m each), both live URLs still responded correctly afterward.

**Microsoft Foundry** — asked about separately. Confirmed eligible (the Free Trial subscription can create
Azure AI Foundry/OpenAI resources — verified by actually creating and deleting test ones), but not adopted:
the affordable pay-per-token pricing tier for `gpt-4o-mini` only exists in `eastus`, not the Indian regions
everything else lives in, and switching would mean either cross-region latency or moving the whole
deployment, plus real code changes (swapping the OpenAI client for `AzureOpenAI` in 4 files) with no
concrete need driving it. Everything still calls OpenAI's API directly, as it always has.

**Full end-to-end verification**: `node driver.mjs` (the project's own Playwright driver — see
`.claude/skills/run-paynexus/SKILL.md`) run against the *real deployed URLs*
(`FRONTEND_URL=https://nice-desert-0837ea310.7.azurestaticapps.net`), not localhost — register → tab
renders → payslip entered → question asked → real answer + table rendered, **PASS, zero console errors**.

Real ongoing cost: ~$34/month (Postgres B1MS + App Service B1; Static Web Apps and Docker Hub are both
free tier), against a $200 free-trial credit with a hard spending limit — Azure disables resources rather
than charging a card if the credit or trial period runs out.
